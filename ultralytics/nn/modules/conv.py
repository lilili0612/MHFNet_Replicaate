...
class DCN(nn.Module):
    ...
    # 【修改点3】同样将这里的默认参数 gc=8 改为 gc=16
    def __init__(self, c, k=3, s=1, p=None, d=1, gc=16, dk=3, act=True):
        super().__init__()
        assert k == 3

        gc = 16  # 强制保护
        self.s = s  # 记录 stride，后面恢复形状需要用到
    ...
    def forward(self, x):
        ...
        # 1. 转换形状: (Batch, Channels, Height, Width) -> (Batch, Length, Channels)
        x_reshaped = x.view(B, C, H * W).permute(0, 2, 1).contiguous()

        # 2. 送入 DCNv4 前向传播
        try:
            # 官方原版 DCNv4 通常需要 shape 参数来计算变形网格
            out = self.conv(x_reshaped, shape=(H, W))
        except TypeError:
            # 兼容某些去掉了 shape 参数的魔改版 DCNv4
            out = self.conv(x_reshaped)

        # 3. 根据步长 (stride) 计算输出特征图的真实宽高
        H_out = math.ceil(H / self.s)
        W_out = math.ceil(W / self.s)

        # 4. 转换回传统的 4D 图像形状: (Batch, Length_out, Channels) -> (Batch, Channels, H_out, W_out)
        out = out.permute(0, 2, 1).contiguous().view(B, C, H_out, W_out)
